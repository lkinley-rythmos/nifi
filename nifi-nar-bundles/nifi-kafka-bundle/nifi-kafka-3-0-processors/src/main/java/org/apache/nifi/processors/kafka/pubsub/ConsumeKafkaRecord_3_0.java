...
@CapabilityDescription("Consumes messages from Apache Kafka specifically built against the Kafka 3.0 Consumer API. "
    + "The complementary NiFi processor for sending messages is PublishKafkaRecord_3_0. Please note that, at this time, the Processor assumes that "
...
@Tags({"Kafka", "Get", "Record", "csv", "avro", "json", "Ingest", "Ingress", "Topic", "PubSub", "Consume", "3.0"})
@WritesAttributes({
...
@SeeAlso({ConsumeKafka_3_0.class, PublishKafka_3_0.class, PublishKafkaRecord_3_0.class})
public class ConsumeKafkaRecord_3_0 extends AbstractProcessor implements VerifiableProcessor {
    ...
    protected ConsumerPool createConsumerPool(final ProcessContext context, final ComponentLog log) {
        ...
        final String topicListing = context.getProperty(ConsumeKafkaRecord_3_0.TOPICS).evaluateAttributeExpressions().getValue();
        final String topicType = context.getProperty(ConsumeKafkaRecord_3_0.TOPIC_TYPE).evaluateAttributeExpressions().getValue();
